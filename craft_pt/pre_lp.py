...
import craft_pt.file_utils_lp as file_utils
import json
import zipfile
...
def lp_pre(net, img, args):
    ...
    try:
        img, tof = file_utils.saveResult('', image[:,:,::-1], polys)
...
def main():
    ...
    # load data
    # df = pd.DataFrame(columns=['lp_list'])
    img_dict = {}
    for k, image_path in enumerate(image_list):
        ...
        bboxes, polys, score_text = test_net(net, image, args.text_threshold, args.link_threshold, args.low_text, args.cuda, args.poly, refine_net=None)
        ...
        # cv2.imwrite(mask_file, score_text)
        try:
            img, tof = file_utils.saveResult(image_path, image[:,:,::-1], polys)
            print("elapsed time : {}s".format(time.time() - t))
            if img is not None:
                img_dict.update({'{}'.format(image_path): img})
            print(tof)
            # if tof:
                # print('......')
                # df = df.append({'lp_list': filename}, ignore_index=True)
        except Exception as e:
            print(e)
            print(image_path)
        # cv2.imshow('1', image)
        # cv2.imshow('3', img)
        # cv2.moveWindow("1",500,250)#將視窗移到指定位置
        # # cv2.moveWindow("2",500,500)   #將視窗移到指定位置     
        # cv2.moveWindow("3",500,750)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
    # df.to_csv('lp_list.csv', header=None)
    return img_dict
    # print("elapsed time : {}s".format(time.time() - t))
...
if __name__ == '__main__':
    ...
    #     cv2.imwrite(mask_file, score_text)
    #     try:
    #         img, tof = file_utils.saveResult(image_path, image[:,:,::-1], polys, dirname=result_folder)
    #         print(tof)
    #         if tof:
    #             # print('......')
    #             df = df.append({'lp_list': filename}, ignore_index=True)
